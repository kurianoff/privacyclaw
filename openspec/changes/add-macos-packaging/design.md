...
- A `--tray` CLI flag enables tray mode; the `.app` bundle's launch script passes this flag automatically. Plain `privacyclaw start` from a terminal works unchanged.
...
- `privacyclaw network-enable` writes a pf anchor at `/etc/pf.anchors/privacyclaw`, adds an include to `/etc/pf.conf` if not present, and calls `pfctl -f /etc/pf.conf`.
- For boot persistence it installs a LaunchDaemon at `/Library/LaunchDaemons/com.privacyclaw.pf.plist` (via the same admin auth) that re-applies the anchor on boot.
- `privacyclaw network-disable` reverts all of the above (removes anchor, removes include, unloads and removes the LaunchDaemon).
- **Brew vs .pkg difference**: the `.pkg` postinstall script calls `network-enable` automatically (with admin auth granted by the installer); brew users call `privacyclaw network-enable` once manually or from the dashboard toggle. Behaviour and outcome are identical.
...
- Stored at `~/.config/privacyclaw/bin/llama-server` on first launch (extracted from bundle if not present).
...
2. **Homebrew formula** — CLI binary + bundled `llama-server` resource. `brew services start privacyclaw` manages the LaunchAgent. No tray app.
...
- The tap is a separate `homebrew-privacyclaw` Git repository (can be a private repo or hosted on the same GitHub account).
...
- **llama-server version drift**: the pinned binary may lag behind llama.cpp releases. A future `privacyclaw update-llama-server` command can refresh it.
- **tray-icon macOS event loop**: `tray-icon` requires `NSApplication` on macOS. If Apple changes the API this may break. Mitigation: the tray is purely additive; `privacyclaw start` without `--tray` is unaffected.
...
- Homebrew tap hosting: private tap on GitHub (`github.com/<user>/homebrew-privacyclaw`) is sufficient for now. Public tap can be a follow-up if the tool is open-sourced.
