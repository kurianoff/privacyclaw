...
The project SHALL produce a macOS `.pkg` installer that installs privacyclaw as a ready-to-use system service. The installer SHALL include the privacyclaw binary, the `llama-server` universal binary, and the macOS `.app` bundle. A postinstall script SHALL: generate the CA if not already present, add it to the System keychain with full trust, install a LaunchAgent at `~/Library/LaunchAgents/com.privacyclaw.proxy.plist` to start the proxy at login, and print a summary of installed components to the installer log.
...
- **WHEN** the installer runs and a CA already exists at `~/.config/privacyclaw/ca/ca.pem`
- **THEN** the postinstall script skips CA generation and keychain installation
- **AND** existing conversation history is preserved

#### Scenario: Uninstall

- **WHEN** the user runs `privacyclaw uninstall` or a provided uninstall script
...
The project SHALL maintain a Homebrew formula in a private tap (`homebrew-privacyclaw`) that installs the privacyclaw CLI binary and the `llama-server` binary as a formula resource. `brew services start privacyclaw` SHALL manage the proxy as a LaunchAgent.

#### Scenario: brew install and start

- **WHEN** the user runs `brew install <tap>/privacyclaw && brew services start privacyclaw`
- **THEN** the proxy starts on ports 16440 (HTTP) and 16443 (dashboard)
- **AND** `privacyclaw` is available on PATH

#### Scenario: brew services stop

- **WHEN** the user runs `brew services stop privacyclaw`
- **THEN** the proxy process terminates and the LaunchAgent is unloaded

### Requirement: Homebrew Cask (.app bundle)

The project SHALL maintain a Homebrew cask (`privacyclaw-app`) that installs the `.app` bundle via a `.dmg`. The cask SHALL place the app in `/Applications/Privacyclaw.app` and link the CLI binary to `/usr/local/bin/privacyclaw`.

#### Scenario: cask install and launch

- **WHEN** the user runs `brew install --cask <tap>/privacyclaw-app` and opens the app
...
The project SHALL produce a macOS `.app` bundle with `LSUIElement = true` (agent app: no Dock icon). The bundle SHALL contain the privacyclaw binary, llama-server binary, and app icon. On launch the app SHALL start the HTTP proxy and dashboard, then show the menu bar icon. The bundle SHALL be buildable with `make app` and packageable with `make pkg`.

#### Scenario: Launch from Applications

- **WHEN** the user double-clicks `Privacyclaw.app`
...
- **AND** `~/.config/privacyclaw/bin/llama-server` does not exist
...
A LaunchAgent plist SHALL be provided at `com.privacyclaw.proxy.plist`. It SHALL start `privacyclaw start` at login with stdout/stderr directed to `~/.config/privacyclaw/logs/proxy.log`. It SHALL be installed by the `.pkg` postinstall script and by the Homebrew formula's `brew services` integration.

#### Scenario: Proxy starts at login

- **WHEN** the user logs into macOS after installation
- **THEN** the privacyclaw proxy starts automatically within 5 seconds
- **AND** log output is written to `~/.config/privacyclaw/logs/proxy.log`

#### Scenario: LaunchAgent restart on crash

- **WHEN** the privacyclaw process exits unexpectedly
- **THEN** launchd restarts it after a 5-second delay (KeepAlive = true)

### Requirement: Network Privilege Helper

`privacyclaw network-enable` and `privacyclaw network-disable` SHALL manage two system resources that together route intercepted LLM API traffic through the network proxy: `/etc/hosts` entries for the configured intercept domains, and macOS pf rules to redirect port 443 TCP traffic to the network proxy port (16441). Before making any changes the system SHALL snapshot the original state of both resources so they can be restored exactly. Both commands SHALL use `osascript` to request admin credentials via a single native dialog. The same mechanism is used whether privacyclaw was installed via `.pkg` or Homebrew. The `.pkg` postinstall script MAY call `network-enable` automatically if the user opted in during install.

#### Scenario: network-enable applies /etc/hosts entries and pf rules

- **WHEN** the user runs `privacyclaw network-enable` (or clicks the dashboard/menu toggle)
- **THEN** a single native macOS admin credentials dialog appears
- **AND** on success, the original `/etc/hosts` content is saved to `~/.config/privacyclaw/backup/hosts.bak`
- **AND** entries mapping each intercept domain to `127.0.0.1` are appended to `/etc/hosts` (marked with `# privacyclaw`)
- **AND** `/etc/pf.anchors/privacyclaw` is written with the port 443 → 16441 redirect rule
- **AND** the original `/etc/pf.conf` is saved to `~/.config/privacyclaw/backup/pf.conf.bak`
- **AND** the anchor include is appended to `/etc/pf.conf` and the ruleset is reloaded
- **AND** a LaunchDaemon at `/Library/LaunchDaemons/com.privacyclaw.pf.plist` is installed and loaded for boot persistence

#### Scenario: network-disable reverts /etc/hosts and pf rules

- **WHEN** the user runs `privacyclaw network-disable`
- **THEN** a native macOS admin credentials dialog appears
- **AND** on success, all lines marked `# privacyclaw` are removed from `/etc/hosts`
- **AND** the pf anchor is flushed and the include removed from `/etc/pf.conf`
- **AND** the LaunchDaemon is unloaded and deleted
- **AND** the backup files in `~/.config/privacyclaw/backup/` are removed

#### Scenario: network-enable is idempotent

- **WHEN** `privacyclaw network-enable` is run when rules and hosts entries are already active
...
2. Unload and remove the LaunchAgent (`com.privacyclaw.proxy.plist`)
3. If network proxy was enabled: revert `/etc/hosts` (remove all `# privacyclaw` lines), flush the pf anchor, remove the pf include from `/etc/pf.conf`, unload and remove the pf LaunchDaemon (`com.privacyclaw.pf.plist`)
4. Remove the CA certificate from the System keychain
5. Remove the binary from `/usr/local/bin/privacyclaw`
6. Remove `/Applications/Privacyclaw.app` (if present)
7. Remove `~/.config/privacyclaw/bin/llama-server`
8. With `--purge`: remove `~/.config/privacyclaw/` entirely (models, logs, DB, config, CA, backups)
...
- **WHEN** the user runs `privacyclaw uninstall` and grants admin credentials
...
- **AND** `~/.config/privacyclaw/` (logs, DB, models, config) is left intact
...
- **AND** `~/.config/privacyclaw/` is removed entirely
- **AND** the summary confirms "All privacyclaw data deleted"

#### Scenario: Uninstall when network proxy was never enabled

- **WHEN** the user runs `privacyclaw uninstall` and network proxy was never enabled
...
- **WHEN** `privacyclaw uninstall` is run while the proxy process is active
