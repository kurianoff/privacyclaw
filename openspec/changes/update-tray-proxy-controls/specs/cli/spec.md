...
- **AND** a macOS menu bar icon appears showing a green-eye icon (running state)
- **AND** the menu contains: Stop Proxy, HTTP Proxy (checked), Network Proxy, PII Protection, Open Dashboard, Quit Privacyclaw

#### Scenario: tray stop proxy

- **WHEN** the user clicks "Stop Proxy" in the tray menu
- **THEN** the HTTP CONNECT listener is stopped
- **AND** if network proxy routing is active it is disabled
- **AND** the tray icon changes to a red-eye icon
- **AND** HTTP Proxy, Network Proxy, and PII Protection menu items become disabled
- **AND** Open Dashboard remains enabled (dashboard continues running)
- **AND** the menu item label changes to "Start Proxy"

#### Scenario: tray start proxy

- **WHEN** the user clicks "Start Proxy" in the tray menu (proxy is stopped)
- **THEN** the HTTP CONNECT listener is started (HTTP Proxy toggle defaults to ON)
- **AND** the tray icon changes to a green-eye icon
- **AND** HTTP Proxy, Network Proxy, and PII Protection menu items become enabled
- **AND** the menu item label changes to "Stop Proxy"

#### Scenario: tray http proxy toggle on

- **WHEN** the proxy is running and the user clicks "HTTP Proxy" (currently unchecked)
- **THEN** the HTTP CONNECT listener is started
- **AND** the HTTP Proxy menu item becomes checked

#### Scenario: tray http proxy toggle off

- **WHEN** the proxy is running and the user clicks "HTTP Proxy" (currently checked)
- **THEN** the HTTP CONNECT listener is stopped
- **AND** the HTTP Proxy menu item becomes unchecked
- **AND** the dashboard remains accessible

#### Scenario: tray dashboard always accessible

- **WHEN** the user clicks "Open Dashboard" regardless of proxy running state
- **THEN** the dashboard URL opens in the default browser
- **AND** the dashboard is accessible because it runs for the entire tray process lifetime

#### Scenario: tray quit

- **WHEN** the user clicks "Quit Privacyclaw"
- **THEN** Stop Proxy sequence is executed (HTTP listener aborted, network routing disabled if active)
- **AND** the LaunchAgent is unloaded
...
### Requirement: Tray Icon Status Indicator

The macOS tray icon SHALL reflect the current proxy running state via a colour-coded centre dot.

#### Scenario: Running state icon

- **WHEN** the proxy is running (HTTP listener active)
- **THEN** the tray icon shows a green centre dot (RGB 0, 200, 80)

#### Scenario: Stopped state icon

- **WHEN** the proxy is stopped (HTTP listener not active)
- **THEN** the tray icon shows a red centre dot (RGB 200, 50, 50)

#### Scenario: Dashboard-only mode icon

- **WHEN** the proxy is stopped but the dashboard is still running
- **THEN** the tray icon shows the red-dot (stopped) icon
- **AND** the dashboard remains accessible via Open Dashboard

### Requirement: Tray Dashboard Permanent Lifecycle

The dashboard SHALL start at tray launch and remain running until the process exits, regardless of Start/Stop Proxy actions.

#### Scenario: Dashboard survives Stop Proxy

- **WHEN** the user clicks "Stop Proxy"
- **THEN** the dashboard is still reachable at its configured URL
- **AND** `GET /api/config` returns a valid response

#### Scenario: Dashboard starts before proxy on tray launch

- **WHEN** `privacyclaw start --tray` is invoked
- **THEN** the dashboard listener binds before the first HTTP CONNECT listener spawn
- **AND** the dashboard URL is accessible even if the user immediately clicks "Stop Proxy"
