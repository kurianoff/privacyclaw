...
fn info_plist_has_correct_bundle_id() {
    let plist = make_info_plist("0.1.0");
    assert!(
        plist.contains("<key>CFBundleIdentifier</key><string>com.privacyclaw.app</string>"),
        "Info.plist must contain CFBundleIdentifier = com.privacyclaw.app"
    );
}
...
/// §8.T4: Info.plist sets bundle executable to privacyclaw-app.
#[test]
fn info_plist_executable_is_privacyclaw_app() {
    let plist = make_info_plist("0.1.0");
    assert!(
        plist.contains("<key>CFBundleExecutable</key><string>privacyclaw-app</string>"),
        "Info.plist CFBundleExecutable must be privacyclaw-app"
    );
}
...
fn cmd_stop_with_no_pid_file_is_noop() {
    ...
    let absent = dir.path().join("privacyclaw.pid");
