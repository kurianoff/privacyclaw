...
//
// These tests exercise the public API exported from privacyclaw::pii and
// privacyclaw::config.  They do not require a running proxy, model files, or
// network access — all data is synthetic.

use privacyclaw::config::PiiConfig;
use privacyclaw::parser::Provider;
use privacyclaw::pii::vault::{PiiSpan, PiiType, PiiVault};
use privacyclaw::pii::{Locale, PiiPipeline};
...
fn activate_not_downloaded_model_returns_false() {
    use privacyclaw::models::is_downloaded;
...
async fn t3_standalone_config_path() {
    use privacyclaw::config::{Config, ConfigManager, is_t3_standalone};
