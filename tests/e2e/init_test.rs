/// E2E tests for `privacyclaw init`.
///
...
fn run_init() {
    let status = Command::new(binary_path())
        .arg("init")
        .status()
        .expect("privacyclaw init");
    assert!(status.success(), "privacyclaw init exited with failure");
}

// ── Tests ─────────────────────────────────────────────────────────────────────

/// After `privacyclaw init`, ca.pem and ca.key.pem must exist in the CA directory.
#[test]
...
/// Running `privacyclaw init` twice must NOT overwrite an existing CA.
...
fn test_init_is_idempotent() {
    ...
    assert_eq!(
        before, after,
        "privacyclaw init regenerated the CA on a second call (should be idempotent)"
    );
}

/// `privacyclaw ca-path` must exit 0 and print a path ending in `.pem`.
#[test]
...
/// `privacyclaw --version` exits 0 and prints a semantic version number.
#[test]
fn test_version_flag() {
    let output = Command::new(binary_path())
        .arg("--version")
        .output()
        .expect("privacyclaw --version");
    assert!(output.status.success());
    let out = String::from_utf8_lossy(&output.stdout);
    // Expect something like "privacyclaw 0.2.0" or just "0.2.0"
    assert!(
        out.contains('.'),
...
/// `privacyclaw setup-network` prints /etc/hosts entries for known LLM domains.
#[test]
fn test_setup_network_prints_hosts_entries() {
    ...
    let output = Command::new(binary_path())
        .arg("setup-network")
        .output()
        .expect("privacyclaw setup-network");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
...
/// `privacyclaw test-pii` detects an email address in a plain-text string.
#[test]
fn test_pii_detection_email() {
    let output = Command::new(binary_path())
        .args(["test-pii", "Reach me at jane.doe@example.com anytime."])
        .output()
        .expect("privacyclaw test-pii");
    assert!(
        output.status.success(),
...
/// `privacyclaw test-pii` detects a US phone number.
#[test]
...
/// `privacyclaw stop` when nothing is running must exit gracefully (no panic, no hard error).
#[test]
fn test_stop_when_not_running_is_graceful() {
    // Remove any lingering PID file first so we get a clean "not running" path.
    let pid_path = {
        let home = std::env::var("HOME").expect("HOME");
        std::path::PathBuf::from(home).join(".config/privacyclaw/privacyclaw.pid")
    };
...
/// `privacyclaw init --help` mentions the --network flag.
#[test]
fn test_init_help_mentions_network_flag() {
    let output = Command::new(binary_path())
        .args(["init", "--help"])
        .output()
        .expect("privacyclaw init --help");
    let combined = format!(
        "{}{}",
    ...
    assert!(
        ...
        "init --help does not mention --network: {combined}"
    );
    assert!(
        combined.contains("pf") || combined.contains("network proxy"),
        "init --help missing pf/network proxy description: {combined}"
    );
}

/// `privacyclaw init --network` flag is accepted (not an unknown flag).
///
/// The privileged pf write is expected to fail in CI (no admin dialog or osascript).
/// This test only verifies the flag is parsed and that the failure, if any,
/// is a network-setup error rather than "unrecognized argument".
#[test]
fn test_init_network_flag_is_accepted() {
    let output = Command::new(binary_path())
        .arg("init")
        .arg("--network")
        .output()
        .expect("privacyclaw init --network");
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    ...
    assert!(
        ...
        "init --network was rejected as unknown flag: {combined}"
    );
    // Success or a privilege/network-setup error are both acceptable.
    // A panic or arg-parse error is not.
    assert!(
        !combined.contains("thread 'main' panicked"),
        "init --network caused a panic: {combined}"
    );
}

/// `privacyclaw uninstall --help` mentions --purge.
#[test]
fn test_uninstall_help_mentions_purge() {
    let output = Command::new(binary_path())
        .args(["uninstall", "--help"])
        .output()
        .expect("privacyclaw uninstall --help");
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
