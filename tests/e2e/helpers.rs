...
/// Path to the privacyclaw binary being tested.
/// `CARGO_BIN_EXE_privacyclaw` is set by Cargo when running `cargo test`.
pub fn binary_path() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_privacyclaw"))
}

/// Default CA directory used by privacyclaw (matches `default_ca_dir()` in config.rs).
...
/// away from privacyclaw's default ports 16440/16441/16443).
...
/// Write a minimal privacyclaw config.toml into `dir`.
...
/// Run `privacyclaw init` exactly once per test-binary invocation.
...
pub fn ensure_init() {
    ...
    DONE.get_or_init(|| {
        ...
        assert!(status.success(), "privacyclaw init failed in ensure_init");
    });
}

// ── ProxyHandle ───────────────────────────────────────────────────────────────

/// A running privacyclaw proxy process with isolated config.
...
impl ProxyHandle {
    /// Spawn `privacyclaw start` with a fresh temp config (unique ports, temp storage).
    ...
    pub fn start() -> Self {
        ...
        let child = Command::new(binary_path())
            ...
            .stderr(Stdio::null())
            .spawn()
            .expect("spawn privacyclaw start");
