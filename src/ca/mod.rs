...
pub fn generate_ca(ca_dir: &Path) -> Result<CaBundle> {
    ...
    params.distinguished_name.push(DnType::OrganizationName, "Privacyclaw Privacy Proxy");
    params.distinguished_name.push(DnType::CommonName, "Privacyclaw Root CA");
...
pub fn install_ca_trust(ca_dir: &Path) -> Result<()> {
    ...
    #[cfg(target_os = "linux")]
    {
        let dest = std::path::PathBuf::from("/usr/local/share/ca-certificates/privacyclaw-ca.crt");
...
fn print_manual_instructions(cert_path: &Path) {
    ...
    println!("  Linux:   sudo cp {:?} /usr/local/share/ca-certificates/privacyclaw-ca.crt && sudo update-ca-certificates", cert_path);
