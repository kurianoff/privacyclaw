//! `privacyclaw uninstall [--purge]` — ordered step orchestrator.
//!
...
impl UninstallRunner {
    ...
    pub fn run(&self) -> Vec<StepResult> {
        ...
        // Network proxy cleanup — only if privacyclaw entries are active.
    ...
    fn step_remove_privileged(&self) -> StepResult {
        ...
        let script = format!(
            ...
            "security remove-trusted-cert -d {ca} 2>/dev/null || true; \
             security delete-certificate -c 'Privacyclaw Root CA' -t 2>/dev/null || true; \
             # Unload and remove pf LaunchDaemon.
             launchctl bootout system /Library/LaunchDaemons/com.privacyclaw.pf.plist 2>/dev/null || \
               launchctl unload /Library/LaunchDaemons/com.privacyclaw.pf.plist 2>/dev/null || true; \
             rm -f /Library/LaunchDaemons/com.privacyclaw.pf.plist; \
             # Remove binary and shared files.
             rm -f /usr/local/bin/privacyclaw; \
             rm -rf /usr/local/share/privacyclaw; \
             # Forget the package receipt so Installer shows a clean state.
             pkgutil --forget com.privacyclaw.pkg 2>/dev/null || true",
            ca = ca_str,
        );
...
fn launch_agent_path() -> std::path::PathBuf {
    dirs::home_dir()
        ...
        .join("Library/LaunchAgents/com.privacyclaw.proxy.plist")
}
