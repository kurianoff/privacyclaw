...
async fn apply_protection_level(
    ...
    if needs_model {
        ...
        // Print the manual llama-server command; privacyclaw start manages the
        ...
        println!("Or simply run `privacyclaw start` — it will launch llama-server automatically.");
        tracing::info!(
...
async fn set_config(cfg_mgr: Arc<ConfigManager>, assignment: &str) -> Result<()> {
    ...
    if result.restart_required {
        println!("Note: restart privacyclaw for this change to take effect.");
    }
    Ok(())
}
