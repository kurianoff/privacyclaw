...
Claudovka's PII pipeline was designed to protect **personal identifiable information** with a fixed structure: credentials, contact details, identity documents. It does this well.
...
These are different threat models. Claudovka addresses the first well. It is not a confidentiality boundary for the second.

---

## Additional risks regardless of PII protection

- **Local conversation log is unencrypted.** The full original text (including any PII that was sanitised outbound) is stored in `~/.config/claudovka/` in plain NDJSON. Anyone with filesystem access can read it.
...
For users concerned about **personal credential and contact data leakage**, Claudovka with all tiers enabled provides meaningful protection.
