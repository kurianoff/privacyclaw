...
/// They are gated behind the `CLAUDOVKA_TEST_NETWORK` environment variable:
///
///   sudo CLAUDOVKA_TEST_NETWORK=1 cargo test --test network_proxy_test
///
...
/// (including `claudovka network-enable`) also run as root and can write to
...
/// Set `CLAUDOVKA_TEST_NETWORK=1` (and run as root via sudo).
fn network_tests_enabled() -> bool {
    std::env::var("CLAUDOVKA_TEST_NETWORK")
...
macro_rules! skip_unless_network {
    () => {
        if !network_tests_enabled() {
            println!(
                "SKIP (set CLAUDOVKA_TEST_NETWORK=1 and run as root to enable network proxy tests)"
            );
            return;
        }
    };
...
/// `claudovka network-enable` writes /etc/hosts entries for all default LLM domains.
#[test]
...
fn test_network_enable_creates_pf_anchor() {
    ...
    let anchor_path = std::path::Path::new("/etc/pf.anchors/claudovka");
    assert!(
        anchor_path.exists(),
        "/etc/pf.anchors/claudovka does not exist after network-enable"
    );
...
/// After `network-enable`, pfctl reports active rules for the claudovka anchor.
#[test]
fn test_network_enable_loads_pf_rules() {
    ...
    // pfctl -a claudovka -s rules → prints the active redirect rules.
    let output = Command::new("pfctl")
        .args(["-a", "claudovka", "-s", "rules"])
        .output()
...
/// `claudovka network-disable` removes all claudovka entries from /etc/hosts.
#[test]
fn test_network_disable_cleans_hosts() {
    ...
    assert!(
        hosts_before.contains("# claudovka"),
    ...
    assert!(
        !hosts_after.contains("# claudovka"),
        "/etc/hosts still contains claudovka entries after disable:\n{hosts_after}"
    );
}

/// `claudovka network-disable` flushes the pf anchor (no active redirect rules).
#[test]
...
/// `claudovka network-enable` is idempotent — running it twice does not
...
fn test_network_enable_is_idempotent() {
    ...
    // Count occurrences of "api.anthropic.com  # claudovka".
    let count = hosts
        .lines()
        .filter(|l| l.contains("api.anthropic.com") && l.contains("# claudovka"))
        .count();
    assert_eq!(
        count, 1,
...
/// After `network-enable`, `claudovka network-disable` followed by another
...
fn test_network_enable_disable_enable_roundtrip() {
    ...
    assert!(
        hosts.contains("# claudovka"),
        "/etc/hosts missing claudovka entries after roundtrip enable"
    );
}
...
fn test_full_network_proxy_lifecycle() {
    ...
    assert!(hosts.contains("# claudovka"), "hosts missing claudovka entries");
