...
use claudovka::pii::inject_system_instruction;
use claudovka::pii::tier3::SlmSidecar;
use claudovka::pii::vault::{PiiVault, PiiType};
use claudovka::pii::{PiiPipeline, SYSTEM_REMINDER};
...
async fn t3_standalone_outbound_populates_vault() {
    ...
    let result = pipeline
        .process_request_body_async(&body_bytes, &vault_handle, Provider::Anthropic, &claudovka::pii::Locale::EnUs)
        .await;
