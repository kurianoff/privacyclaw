# Claudovka Usage Guide

Claudovka is a local MITM proxy that sits between your AI coding tools (Claude Code, Cursor, etc.) and LLM APIs. It lets you inspect all AI traffic in a dashboard, and optionally detect and replace PII before it ever leaves your machine.
...
```bash
export HTTPS_PROXY=http://127.0.0.1:16440
export NODE_EXTRA_CA_CERTS="$HOME/.config/claudovka/ca/ca.pem"
```
...
Edit (or create) `~/.config/claudovka/config.toml`:
...
```bash
claudovka test-pii "My email is john@example.com and SSN is 123-45-6789"
```
...
JSON output:
```bash
claudovka test-pii --format json "email: john@example.com"
```

Test with a specific locale:
```bash
claudovka test-pii --locale en-GB "NI number: AB 12 34 56 C"
```
...
Set `RUST_LOG=claudovka=debug` for more verbose output including per-chunk replacement buffer activity.

---

## Vault persistence

Per-conversation PII mappings survive proxy restarts. If you stop and restart claudovka mid-conversation, the same synthetic tokens are used for the same originals — the LLM context remains consistent.

Vaults are stored alongside conversation files in the storage directory (default `~/.config/claudovka/`). Each vault is a single line appended to the conversation's NDJSON file, tagged `"type":"vault"`.
...
- **The local conversation log contains original PII.** Only outbound LLM traffic is sanitised. Your local storage (`~/.config/claudovka/`) is not encrypted — treat it accordingly.
